...
LiveData의 경우 오직 RxJava의 `Subject` 와 같이 Cold한 Observable형태로 이루어져 있어 구독(`observe()`) 를 해야만 구독자가 배출 된 데이터를 받아볼 수 있습니다.



당연하게도 실무에서도 마찬가지로 LiveData의 장점을 인지하고 있고, 앞으로 나올 다양한 연산자들과 RxJava 라이브러리의 대항마로 공식적으로 곧 선보일 Flow가 서서히 RxJava를 넘어 대세가 될 것이라는 예감을 갖고있습니다.
